...
public class GridManager : MonoBehaviour {
    ...
    float jumpSpeed;
    [SerializeField]
    float popVisualDuration;
	[SerializeField]
	float preclickDuration = 0.4f;
	[Space]
    [SerializeField]
    Cell cellPrefab;
    [SerializeField]
    MatchableTile matchableTilePrefab;
    ...
    Dictionary<SpecialTileType, System.Collections.Generic.HashSet<Coordinate>> postPopSpecialTiles = new Dictionary<SpecialTileType, System.Collections.Generic.HashSet<Coordinate>>();
    Dictionary<SpecialTileType, System.Collections.Generic.HashSet<Tile>> postMoveSpecialTiles = new Dictionary<SpecialTileType, System.Collections.Generic.HashSet<Tile>>();
    ...
    MatchType GetRandomMatchType(System.Collections.Generic.HashSet<MatchType> matchTypes, bool typesAreExcluded) {
    ...
    public void AddPostPopSpecial(SpecialTileType st, Coordinate c) {
        if (!postPopSpecialTiles.ContainsKey(st))
            postPopSpecialTiles.Add(st, new System.Collections.Generic.HashSet<Coordinate>());
    ...
    public void AddPostMoveSpecial(SpecialTileType st, Tile t) {
        if (!postMoveSpecialTiles.ContainsKey(st))
            postMoveSpecialTiles.Add(st, new System.Collections.Generic.HashSet<Tile>());
    ...
    Dictionary<int, System.Collections.Generic.HashSet<int>> GetHoles() {
        if (gridSettings.specificHoles.Length == 0 && gridSettings.randomHoles == 0)
            return null;
        Dictionary<int, System.Collections.Generic.HashSet<int>> holes = new Dictionary<int, System.Collections.Generic.HashSet<int>>();
        System.Collections.Generic.HashSet<int> indices = new System.Collections.Generic.HashSet<int>();
        int edgeHoles = 0;
        foreach (Coordinate c in gridSettings.specificHoles) {
            if (c.x == 0 || c.x == gridSettings.gridWidth - 1 || c.y == 0 || c.y == gridSettings.gridHeight - 1) {
                if (!holes.ContainsKey(c.x))
                    holes.Add(c.x, new System.Collections.Generic.HashSet<int>());
        ...
        foreach (int i in indices) {
            ...
            if (!holes.ContainsKey(x))
                holes.Add(x, new System.Collections.Generic.HashSet<int>());
    ...
    public void CreateGrid() {
        Dictionary<int, System.Collections.Generic.HashSet<int>> acellularCoordinates = GetHoles();
    ...
    Tile InstantiateTile(Tile t, int x, int y, bool toTheTop = false, System.Collections.Generic.HashSet<MatchType> matchTypes = null, bool typesAreExcluded = true) {
        return InstantiateTile(t, GetCell(x, y), toTheTop, matchTypes, typesAreExcluded);
    }

    Tile InstantiateTile(Tile t, Cell c, bool toTheTop = false, System.Collections.Generic.HashSet<MatchType> matchTypes = null, bool typesAreExcluded = true) {
        if (t != null) {
    ...
    public void BreakMatches() {
        int change = 0;
        System.Collections.Generic.HashSet<MatchType> adjacentTypes = new System.Collections.Generic.HashSet<MatchType>();
        Cell c;
        for (int x = 0; x < GetGridWidth(); ++x) {
            for (int y = 0; y < GetGridHeight(); ++y) {
                c = GetCell(x, y);
    ...
    bool CheckSameMatchTypeAndAddTypeToSet(Tile t, int x, int y, System.Collections.Generic.HashSet<MatchType> set, bool addNonMatching = false) {
    ...
    IEnumerator TilePopRoutine(List<Dictionary<Tile, Coordinate>> tilesToPop, Callback TilesPopped) {
        ...
        foreach (Coordinate c in specialTilesToAdd.Keys) {
            InstantiateTile(GetRandomSpecialTileToCreate(), c.x, c.y, false, new System.Collections.Generic.HashSet<MatchType>() { specialTilesToAdd[c] }, false).GrowVisual(popVisualDuration);
        }
        TilesPopped();
    }
    ...
    public void GrowTiles(float growPostWait, List<Dictionary<Tile, Coordinate>> tilesToGrow, Callback GrowthDone) {
        Dictionary<Dictionary<Tile, Coordinate>, System.Collections.Generic.HashSet<MatchType>> excludedTypes = new Dictionary<Dictionary<Tile, Coordinate>, System.Collections.Generic.HashSet<MatchType>>();
        foreach (Dictionary<Tile, Coordinate> dic in tilesToGrow) {
            excludedTypes.Add(dic, new System.Collections.Generic.HashSet<MatchType>() { GetMostCommonMatchType(dic) });
        }
        GrowTiles(growPostWait, tilesToGrow, excludedTypes, true, GrowthDone);
    }

    public void GrowTiles(float growPostWait, List<Dictionary<Tile, Coordinate>> tilesToGrow, Dictionary<Dictionary<Tile, Coordinate>, System.Collections.Generic.HashSet<MatchType>> matchTypes, bool typesAreExcluded, Callback GrowthDone) {
        StartCoroutine(GrowthRoutine(growPostWait, tilesToGrow, matchTypes, typesAreExcluded, GrowthDone));
    }

    IEnumerator GrowthRoutine(float growPostWait, List<Dictionary<Tile, Coordinate>> tilesToGrow, Dictionary<Dictionary<Tile, Coordinate>, System.Collections.Generic.HashSet<MatchType>> matchTypes, bool typesAreExcluded, Callback GrowthDone) {
        while (!CanMove) {
            yield return null;
        }
        Cell c;
